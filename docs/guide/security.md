...
app.get('/login-session', (ctx) => {
  ctx.setEncryptedCookie('user_session', { userId: 101, token: 'abc' }, process.env.COOKIE_SECRET);
  return { status: 'logged_in' };
});

app.get('/profile', (ctx) => {
  const session = ctx.getEncryptedCookie('user_session', process.env.COOKIE_SECRET);
  return { session };
});
```
